jQuery(document).ready(function($){    
        ...
        if (requeridos()) {
            event.preventDefault(); 
             $.ajax({
                type: 'post',
	            dataType: 'json',
	            url: url + 'seguridad/add_conceptos', 
	            data: $("#frm_concepto").serialize(),                           
        ...
        if (requeridos()) {
            event.preventDefault(); 
             $.ajax({
                type: 'post',
                dataType: 'json',
                url: url + 'seguridad/editTableConceptos', 
                data: $("#frm_concepto").serialize(),                           
                success: function (result) {
    ...
    $( function() {
       if ( sessionStorage.getItem('insert') != 'false' && sessionStorage.getItem('insert') != null) {
           callToaster('toast-top-right', 'El concepto ha sido creado con éxito','Alerta');           
           sessionStorage.setItem('insert',false)
       }
       if ( sessionStorage.getItem('edit') != 'false' && sessionStorage.getItem('edit') != null) {
           callToaster('toast-top-right', 'El concepto ha sido actualizado con éxito','Alerta','warning');           
